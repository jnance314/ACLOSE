...
def dummy_generate_initial_topics_async(self, cluster_df):
    async def _dummy():
        ...
        unique_clusters = cluster_df[cluster_df['cluster_id'] != -1]['cluster_id'].unique()
...
def labeling_engine():
    ...
    engine = LabelingEngine(llm_settings=llm_settings, sampling_settings=sampling_settings)
    return engine
...
def test_make_model_args(labeling_engine):
    ...
    args = labeling_engine._make_model_args(
        system_prompt, core_points, other_centroids, 
        core_label=None, peripheral_points=None, func="topic-label-pass-2"
    )
...
def test_add_labels_function(monkeypatch):
    ...
    """
    monkeypatch.setattr(LabelingEngine, "generate_initial_topics_async", dummy_generate_initial_topics_async)
    monkeypatch.setattr(LabelingEngine, "update_topics_async", dummy_update_topics_async)
