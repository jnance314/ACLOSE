...
def test_silhouette_fig_missing_reduced_vector(df_missing_reduced_vector):
    ...
    """
    with pytest.raises(
        ValueError, match="No 'reduced_vector' column found in DataFrame"
    ):
...
def test_scatter_fig(dummy_clusters_df):
    ...
    assert isinstance(fig, go.Figure)
    scatter_traces = [
        trace for trace in fig.data if trace.type in ["scatter", "scatter3d"]
    ]
...
def test_validate_scatter_params_invalid_content(dummy_clusters_df):
    ...
    """
    with pytest.raises(
        ValueError, match="content_col_name 'missing_content' not found"
    ):
...
def test_scatter_fig_high_dim(dummy_clusters_high_dim):
    ...
    """
    fig = scatter_fig(
        dummy_clusters_high_dim, content_col_name="content", wrap_width=50
    )
    assert isinstance(fig, go.Figure)
    annotations = fig.layout.annotations
    # At least one annotation should mention that dims > 3 cannot be plotted
    assert any(
        "Cannot plot scatter plot" in annotation.text for annotation in annotations
    )
